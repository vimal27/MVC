...
/// </summary>
public class ExcelGenerate2
{
    public ExcelGenerate2()
    {
    }
    public string ReportGeneration(DataSet ds, string filename, string headername, string sheetname, string Username, bool InitializeRequired = false, int index = -2)
    {
        try
        {
            string result = string.Empty;
            if (InitializeRequired == true)
                InitializeWorkbook(Convert.ToString(filename), Username);
            for (int i = 0; i < ds.Tables.Count; i++)
            {
                if (i == index || index==-2)
                {
                    if (ds.Tables[i] != null)
                    {
                        ...
                        {
                            GenerateData(headername, Convert.ToString(ds.Tables[i].Rows[0][0]), ds.Tables[i], Username);
    ...
    void GenerateData(string headername, string sheetname, DataTable dt, string Username)
    {
        ...
        for (int i = 0; i <= dt.Columns.Count - 1; i++)
        {
            ...
            cell = row.CreateCell(i); cell.CellStyle = hStyle; cell.SetCellValue(Convert.ToString(dt.Columns[i].Caption));
